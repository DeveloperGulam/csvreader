<?php
    ini_set('memory_limit', '1024M');
    require_once __DIR__ . '/bigcommerce.php';
    require_once __DIR__ . '/Akeneo.php';

    $bigcommerce = new Bigcommerce\Product;
    session_start();
    $connection_data = $_SESSION['connection'];
    $akeneo = new Akeneo\Akeneo($connection_data['akeneo_url'], $connection_data['access_token']);
    $product = $bigcommerce->getProduct(1, 1);
    $akeneoProducts = $akeneo->getProduct();
    // $akeneoProduct = getFilteredProducts($akeneoProducts['_embedded']['items'][0]);
    $akeneoProductFields = getFilteredProducts($akeneoProducts['_embedded']['items'][0]);
    // echo "<pre>";
    // print_r($akeneoProductFields);die;


    function getFilteredProducts($akeneoProduct) {
        $filter_products = [];
        foreach(array_slice($akeneoProduct, 1) as $key => $value){
            if(is_array($value)){
                if($key == 'categories' || $key == 'groups'){
                    $newVal = myImpload(",", $value);
                    $filter_products[$key] = $newVal;
                } else {
                    $response = flatArray($value);
                    $filter_products = array_merge($filter_products, $response);
                    // echo "<pre>";
                    // print_r($response);
                }
            } else {
                $filter_products[$key] = $value;
            }
        }
        return $filter_products;
    }

    function flatArray($array) {
        $res = [];
        foreach($array as $key=>$val){
            if(!empty($val[0])) {
                $newVal = myImpload(",", $val, "data");
                $res[$key] = $newVal;
                // echo "<pre>";
                // print_r($newVal);
            }
            // else {
            //     $res[$key] = $val;
            // }
        }
        return $res;
    }

    function myImpload($seprator, $array, $return = null, $condition = null) {
        $res = "";
        $count = 0;
        foreach ($array as $key => $val) {
            $count++;
            if($return != null && !empty($val[$return])){
                if(!is_array($val[$return])){
                    if(!empty($val['locale']) || !empty($val['amount'])) {
                        if(!empty($val['locale']) && $val['locale'] == 'en_US') {
                            $res .=$val[$return];
                        } else if(!empty($val['amount'])) {
                            $condition = ($condition) ? $condition : 'USD';
                            if($val['currency'] == $condition) {
                                $res .=$val[$return];
                            }
                        }
                    } else {
                        if($count != count($array)){
                            $res .=$val[$return] . $seprator;
                        } else {
                            $res .=$val[$return];
                        }
                    }
                } else {
                    $result = myImpload(",", $val[$return], "amount", "USD");
                    if($count != count($array)){
                        $res .=$result . $seprator;
                    } else {
                        $res .=$result;
                    }
                }
            } else {
                if($count != count($array)){
                    $res .=$val . $seprator;
                } else {
                    $res .=$val;
                }
            }
            
        }
        return $res;
    }
    
    // $keys = array_keys_multi($akeneoProduct);
    // $first_key = array_key_first($akeneoProduct['_links']['self']);
    // $flattern = flattern($akeneoProduct);
    // echo "<pre>";
    // print_r(flattern($akeneoProduct));die;
    // echo json_encode($akeneoProduct);die;
    // echo count($flattern);
    // print_r($flattern);
    // print_r($akeneoProduct);
    // die;
    
    // print_r($product['data'][0]);
    
    // $res = array();
    // function flattern($array)
    // {
    //     $flat=[];
    //     foreach($array as $key=>$val){
    //         if(is_array($val)){

    //             // $newKey = $key
    //             // $flat[]=$key;
    //             // echo "<pre>";
    //             // print_r($flat);
    //             // print_r(flattern($val));die;
    //             $flat = array_merge($flat, flattern($val));
    //             // echo "<pre>";
    //             // print_r($flat);
    //         }else{
    //             $flat[$key]=$val;
    //         }
    //     }
    //     return $flat;
    // }

    // function singledimensional($array)
    // {
    //     global $res;
    //     // $res = [];
    //     $level = 0;
    //     foreach ($array as $i => $item) {
    //         if(is_array($item)){
    //             $level++;
    //             $child = singledimensional($item);
    //             // $res[] =  $child;
    //             // print_r($child);die;
    //         } else {
    //             // print_r($item);
    //             // echo "key : $i val: $item <br/>";
    //             $res[$i] = $item;
    //         }
    //     }
    //     return $res;
    // }

    // function array_keys_multi(array $array)
    // {
    //     $keys = array();

    //     foreach ($array as $key => $value) {
    //         $keys[] = $key;

    //         if (is_array($value)) {
    //             $keys = array_merge($keys, array_keys_multi($value));
    //         }
    //     }

    //     return $keys;
    // }
?>
<!DOCTYPE html>
<html>
<head>
    <meta charset='utf-8'>
    <meta http-equiv='X-UA-Compatible' content='IE=edge'>
    <title>Zorang :: Akeneo PIM to BigCommerce</title>
    <meta name='viewport' content='width=device-width, initial-scale=1'>
    <link rel="icon" type="image/ico" href="https://www.akeneo.com/wp-content/themes/akeneo/assets/img/core/favicon.ico">
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/css/bootstrap.min.css" rel="stylesheet">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.2.0/css/all.min.css" />
    <script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/toastr.js/latest/toastr.css" />
    <script src="https://cdnjs.cloudflare.com/ajax/libs/toastr.js/latest/toastr.min.js"></script>
    <style>
        body{
            background:#F3EEF9;
            background-image: url('https://www.akeneo.com/wp-content/themes/akeneo/assets/img/hero-waves/akeneo-ico-waves-hero-default.png');
            background-position: right;
            background-position-y: bottom;
            background-repeat: no-repeat;
        }
        .logoSection{
            margin-top: 15px;
            margin-left: 15px;
        }
        .logoSection label {
            font-style: italic;
            font-family: fantasy;
        }
        .logo {
            height: 35px;
            max-width: 100%;
            transition: .3s;
            width: auto;
        }
        .main {
            padding: 50px;
            margin-top: 20px;
            margin-left: 25%;
            margin-right: 20%;
            margin-bottom: 30px;
            /* border:1px solid #36145E; */
            background:#F3EEF9;
            border-radius:5px;
            box-shadow: 0 0 2px #36145E;
        }
        .main .title {
            margin-top: -20px;
            font-weight: 700;
        }
        .mainly {
            border-top: 1px solid #e3e2e2;
            margin-top: 20px;
            background: #ffffff;
            padding: 20px;
        }
        .field-name{
            font-weight: bold;
        }
        .field-title p {
            font-weight: 700;
        }
        .fields-row {
            margin-top: 20px;
        }
        /* .select-option {
            width: 300px;
        }
        .select-option option {
            width: 300px;
        } */
        .btn-primary {
            background-color: #F3EEF9 !important;
            border: 2px solid #9452ba;
            border-radius: 0.5rem;
            color: #000;
        }
        .btn-primary:hover {
            background-color: #9452ba !important;
            border: 2px solid #9452ba;
            border-radius: 0.5rem;
            color: #fff;
        }
    </style>
</head>
<body>
    <div class="logoSection">
        <img class="logo" src="data:image/png;base64," type="image/jpeg" alt="Akeneo Logo" class="exclude-lazy">
        <label>Zorang Partner</label>
    </div>
    <div class="main">
        <p class="title text-center">Attribute Mapping/Data Sync</p>
        <div class="mainly">
            <div class="row">
                <div class="col-sm-6 field-title border-bottom"><p>AKENEO ATTRIBUTE</p></div>
                <div class="col-sm-6 field-title border-bottom"><p>FIELD IN BIGCOMMERCE</p></div>
            </div>
            <form class="fields-row" action="" method="post" encType="multipart/form-data">
                <?php foreach($product['data'][0] as $field => $value){ ?>
                <div class="row fields-row">
                    <div class="col-sm-6">
                        <select name="data[<?=$field?>]" class="form-control select-option">
                            <option value="">Select</option>
                            <?php foreach($akeneoProductFields as $option => $value){ ?>
                                <option value="<?= $option ?>"><?= $option ?></option>
                            <?php } ?>
                        </select>
                        <!-- <input type="hidden" name="data[bigcommerce][]" value="<?= $field ?>" /> -->
                    </div>
                    <div class="col-sm-6">
                        <p class="field-name"><?= $field ?></p>
                    </div>
                </div>
                <?php } ?>
                <br/>
                <button type="submit" id="submit_attribute" name="submit_attribute" class="btn btn-primary">Import Now</button>
                <a href="#" style="display:none;" type="button" id="downloadBtn" name="downloadBtn" class="btn btn-primary"><i class="fa fa-download"></i> Download CSV</a>
            </form>
        </div>
    </div>
    <?php
    if(isset($_POST['submit_attribute'])){
        $response = $bigcommerce->createProduct();
        echo "<pre>";
        print_r($response);die;
        // $data = $_POST['data'];
        // $akeneo_rows = $_SESSION["akeneo_rows"];
        // $filter_data = [];
        // $filter_data[] = $_SESSION["sample_header"];
        // foreach($akeneo_rows as $key => $value){
        //     $temp_data = [];
        //     foreach($data as $bigcom => $akeneo){
        //         $temp_data[] = (!empty($value[$akeneo])) ? $value[$akeneo] : '';
        //     }
        //     $filter_data[] = $temp_data;
        // }
        // $location = './target/';
        // $file_name = 'target.csv';
        // $file_dir = $location . $file_name;

        // $fp = fopen($file_dir, 'w');
        // foreach ($filter_data as $fields) {
        //     fputcsv($fp, $fields);
        // }
        // fclose($fp);
        // ?>
         <script type="text/javascript">
        //     var fileDir = "<?= $file_dir?>";
        //     document.getElementById('downloadBtn').style.display = 'block';
        //     document.getElementById('downloadBtn').href = fileDir;
        //     document.getElementById('submit_attribute').style.display = 'none';
        // </script>
         <?php
    }
    ?>
</body>
</html>